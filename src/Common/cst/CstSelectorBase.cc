...
template <typename Tria>
void CstSelectorBase<Tria>::clearCache()
{
    __pntCache[0] = INF;
    __pntCache[1] = INF;
    __pntCache[2] = INF;
...
template <typename Tria>
void CstSelectorBase<Tria>::clearFound()
{
    __dofsFound.clear();
...
template <typename Tria>
void CstSelectorBase<Tria>::clear()
{
    __cstsOutput.clear();
...
template <typename Tria>
void CstSelectorBase<Tria>::assignValues(const double,
                                         const double,
                                         const double,
...
template <typename Tria>
void CstSelectorBase<Tria>::init(const std::size_t nDoFs)
{
    __pntCache[0] = INF;
    __pntCache[1] = INF;
    __pntCache[2] = INF;
               ...
               const CstEntry<Tria>& entryRef)
{
    ...
    AssertThrow(dofAtPnt < nDoFs, ExcMessage("The dofAtPnt is larger than nDoFs in addDoF2Cache()."));
...
{
    AssertThrow(hasInit,
                ExcMessage("CstSelectorBase<Tria> is not initialized."));
    ...
    // dofs have already been added into __dofsCache via addDoF2Cache() or addDoFs2Cache()
    ...
    if (hasNewDoF)
    {
        ...
        assignValues(__pntCache[0],
                     __pntCache[1],
                     __pntCache[2],
                     __valuesCache);
        
        AssertThrow(__valuesCache.size() >= nDoFs,
    ...
    // loop over __cstsInput as template to generate __cstPntRecordsFound and __cstsOutput
    for (const CstEntry<Tria>& inputEntry : __cstsInput)
    {
        ...
        AssertThrow(dofAtPnt < __valuesCache.size(),
                    ExcMessage("valuesFunc returned insufficient values for cstEntry.dofAtPnt."));
        
        AssertThrow(std::isfinite(__valuesCache[dofAtPnt]),
                    ExcMessage("Constraint value was not assigned or set an infinite value for dofAtPnt = "
...
{
    AssertThrow(hasInit,
                ExcMessage("CstSelectorBase<Tria> is not initialized. Call preparePntCst() before updateValues()."));
    ...
    {
        AssertThrow(__cstsOutput.empty(),
                    ExcMessage("Internal inconsistency: cstPointRecordsFound() is empty but cstOutput() is not empty."));
        return;
    }
    ...
    for (const CstPointRecord& record : __cstPntRecordsFound)
    {
        ...
        AssertThrow(__valuesCache.size() >= nDoFs,
                    ExcMessage("assignValues()/valuesFunc returned or resized values to a size smaller than nDoFs."));

        for (const std::size_t entryIndex : record.entryIndices)
        {
            AssertThrow(entryIndex < __cstsOutput.size(),
                        ExcMessage("Invalid entry index in CstPointRecord::entryIndices."));

            CstEntryResult<Tria>& entry = __cstsOutput[entryIndex];

            AssertThrow(entry.hasValidDoF(),
                        ExcMessage("CstEntryResult has invalid global DoF during updateValues()."));

            const unsigned int dofAtPnt = entry.getDofAtPnt();

            AssertThrow(dofAtPnt < __valuesCache.size(),
                        ExcMessage("dofAtPnt is out of range of values in updateValues()."));

            AssertThrow(std::isfinite(__valuesCache[dofAtPnt]),
                        ExcMessage("Constraint value was not assigned or is not finite for dofAtPnt = "
                                   + std::to_string(dofAtPnt)
                                   + " in updateValues()."));
