...
void
CstMaker<Tria>
::extractDoFsAtVetex(std::vector<::dealii::types::global_dof_index>& dofs,
...
{
    AssertThrow(!isReady,
                    ExcMessage("Cannot add CstSelector after preparePntCst(). Clear/rebuild CstMaker first."));
...
{
    AssertThrow(isReady,
                ExcMessage("preparePntCst() must be called before updatePntCstValues()."));

    const unsigned int nDoFs = dofHandler.get_fe().dofs_per_vertex;

    AssertThrow(nDoFs > 0,
                ExcMessage("dofs_per_vertex should not be zero in updatePntCstValues()."));
    ...
    for (auto& selectorPtr : __cstSelectors)
    {
        AssertThrow(selectorPtr,
                    ExcMessage("nullptr selector in CstMaker::updatePntCstValues()."));
               ...
               const bool verifyNoRepeatedEntry)
{
    ...
    AssertThrow(isReady, ExcMessage("The csts on points have not been found before applying."));
...
::__verifyInput(const unsigned int nDoFs)
{
    ...
    AssertThrow(nDoFs > 0,
                ExcMessage("dofs_per_vertex should not be zero."));
    ...
    {
        AssertThrow(__cstSelectors[ith],
                    ExcMessage("[internal error] CstSelectorBase<Tria> is nullptr in CstMaker<Tria>."));
        ...
        for (unsigned int k = 0; k < inputEntries.size(); ++k)
        {
            ...
            AssertThrow(used_local_dof_ids.insert(dofAtPnt).second,
                                    ExcMessage(std::to_string(ith)
...
::__verifyAllFound()
{
    ...
    {
        AssertThrow(__cstSelectors[ith],
                    ExcMessage("[internal error] nullptr selector in CstMaker::__verifyAllFound()."));
        ...
        {
            global_n_pnts =
            Utilities::MPI::sum(local_n_pnts,
                                *__mpiInfo.mpiCommPtr());
            
            global_n_entries =
            Utilities::MPI::sum(local_n_entries,
                                *__mpiInfo.mpiCommPtr());
        }
        
        global_total_entries += global_n_entries;
        
        if (selector.expectedNumberOfCstPoints() != CstSelectorBase<Tria>::UNKNOW_SIZE)
        {
            const std::size_t expected_n_pnts =
            selector.expectedNumberOfCstPoints();
            
            AssertThrow(global_n_pnts == expected_n_pnts,
                        ExcMessage(std::to_string(ith)
                                   + "-th constraint selector did not find the expected number of constrained points. "
                                   + "Expected points = "
                                   + std::to_string(expected_n_pnts)
                                   + ", found points = "
                                   + std::to_string(global_n_pnts)
                                   + "."));
        }
        
        if (selector.expectedNumberOfCstEntries() != CstSelectorBase<Tria>::UNKNOW_SIZE)
        {
            const std::size_t expected_n_entries =
            selector.expectedNumberOfCstEntries();
            
            AssertThrow(global_n_entries == expected_n_entries,
                        ExcMessage(std::to_string(ith)
                                   + "-th constraint selector did not find the expected number of constrained entries. "
                                   + "Expected entries = "
                                   + std::to_string(expected_n_entries)
                                   + ", found entries = "
                                   + std::to_string(global_n_entries)
                                   + "."));
        }
        else
        {
            AssertThrow(global_n_entries > 0,
                        ExcMessage(std::to_string(ith)
...
::__verifyNoRepeat()
{
    ...
    {
        AssertThrow(__cstSelectors[ith],
                    ExcMessage("[internal error] nullptr selector in CstMaker::__verifyNoRepeat()."));
    ...
    AssertThrow(global_repetition == 0,
                ExcMessage("Repeated constrained DoF detected in CstMaker. "
                           + (has_local_repetition ?
                              ("Repeated global DoF = "
                               + std::to_string(repeated_dof)
                               + ", selector id = "
                               + std::to_string(repeated_selector_id)
                               + ", entry id = "
                               + std::to_string(repeated_entry_id)
                               + ".") :
                              std::string("The repeated DoF was detected on another MPI rank."))));
}
...
::cstListString(const std::string& name) const
{
    ...
    for (unsigned int ithSelector = 0;
         ithSelector < __cstSelectors.size();
         ++ithSelector)
    {
        ...
        for (unsigned int pointID = 0;
             pointID < records.size();
             ++pointID)
        {
            ...
            for (unsigned int entryID = 0;
                 entryID < record.entryIndices.size();
                 ++entryID)
            {
                const std::size_t outputID = record.entryIndices[entryID];

                AssertThrow(outputID < selector.cstOutput().size(),
                            ExcMessage("Invalid entry index in cstListString()."));

                const CstEntryResult<Tria>& entry = selector.cstOutput()[outputID];
