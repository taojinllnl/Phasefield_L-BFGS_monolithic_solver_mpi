...
#include "Common/cst/CstHelper.h"
...
namespace PhaseField
{
  ...
  /**
   *
   * This class is used to output the results, shortening the length of the
   * main.cc. It includes:
  ...
  public:
    ...
    using CellDataStorage =
      dealii::CellDataStorage<typename Tria::cell_iterator, PointHistory>;

    using DataComponentInterpretationList = std::vector<
  ...
  private:
    ...
    static DataComponentInterpretationList
    ...
    void
    __solution(dealii::DataOut<dim> &data_out, const BVector &solution) const;
  ...
  OutputHelper<LATraits, Tria, PointHistory>::__makeSolutionName()
  {
  ...
  template <typename LATraits, typename Tria, typename PointHistory>
  OutputHelper<LATraits, Tria, PointHistory>::OutputHelper(
    ...
    , __data_component_interpretation(
        OutputHelper<LATraits, Tria, PointHistory>::
          __makeDataComponentInterpretation())
    , __data_component_L2(OutputHelper<LATraits, Tria, PointHistory>::
                            __makeL2DataComponentInterpretation())
    , __solutionName("Case_" + std::to_string(caseID) + "-" +
  ...
  template <typename LATraits, typename Tria, typename PointHistory>
  void
  OutputHelper<LATraits, Tria, PointHistory>::__stressL2(
      ...
      for (unsigned int j = i; j < dim; ++j)
        {
          ...
          VectorTools::project(
            ...
            constraints,
            __qf_cell,
            [&qPntHistory, i, j](const auto        &cell,
          ...
          const std::string stress_name = "Cauchy_stress_" +
                                          std::to_string(i + 1) +
  ...
  template <typename LATraits, typename Tria, typename PointHistory>
  void
  OutputHelper<LATraits, Tria, PointHistory>::__materialIDs(
    dealii::DataOut<dim> &data_out) const
  {
    using namespace dealii;
  ...
  template <typename LATraits, typename Tria, typename PointHistory>
  void
  OutputHelper<LATraits, Tria, PointHistory>::__solution(
    dealii::DataOut<dim> &data_out,
  ...
  template <typename LATraits, typename Tria, typename PointHistory>
  void
  OutputHelper<LATraits, Tria, PointHistory>::__partitioning(
    dealii::DataOut<dim> &data_out) const
  {
    using namespace dealii;
    if constexpr (is_mpi)
  ...
  template <typename LATraits, typename Tria, typename PointHistory>
  void
  OutputHelper<LATraits, Tria, PointHistory>::output(
    const unsigned int     ithTimeStep,
    const unsigned int     polyDegree,
    const std::string     &dir,
    const std::string     &modeType,
    const std::string     &laSolver,
    ...
    const CellDataStorage &qPntHistory) const
  {
    ...
    const std::string filename =
      __solutionName + "_" + modeType + "_" + laSolver + "_" + __mpiType + "-";
    ...
    if constexpr (is_mpi)
      {
        ...
        bcs::CstHelper::cstReinit(constraints,
                                  locally_owned_dofs,
                                  locally_relevant_dofs);
    ...
    __stressL2(data_out, dof_handler_L2, constraints, polyDegree, qPntHistory);
