...
public:
    ...
    using RVertexIter = typename ::dealii::Triangulation<dim, spacedim>::active_vertex_iterator;
...
private:
    ...
    void __verifyInput(const unsigned int nDoFs);
    
    
    void __verifyAllFound();
    
    void __verifyNoRepeat();
...
public:
    ...
    CstMaker& operator=(const CstMaker&) = delete;
    CstMaker& operator=(CstMaker&&) = delete;

    
    void cstReinit(Constraints& constraints,
                   const ::dealii::IndexSet& locally_owned_dofs,
                   const ::dealii::IndexSet& locally_relevant_dofs);
    
    
    void extractDoFsAtVetex(DoFList& dofs,
                            const RVertexIter& vertex,
                            const DoFHandler& dof_handler,
                            const std::size_t nDoFsPerVertex);
    
    void eliminateSelectors();
    CstMaker& addCstSelector(const CstSelectorBase<Tria>& selector);
    bool isAddedSelectors() const;
    
    template <bool isAtBoundary = true>
    void preparePntCst(const Tria&         tria,
                       const DoFHandler&   dofHandler,
                       const bool verifyAllFound = true,
                       const bool verifyNoRepeatedEntry = true);
    
    
    void updatePntCstValues(const DoFHandler& dofHandler,
                            const bool verifyAllFound = true,
                            const bool verifyNoRepeatedEntry = true);
    
    void applyPntCsts(Constraints&        cst,
                      const bool verifyAllFound = true,
                      const bool verifyNoRepeatedEntry = true);
    
    
    void makeCstIfPrepareNeeded(bool&               dofChangedflag,
                                Constraints&        cst,
                                const Tria&         tria,
                                const DoFHandler&   dofHandler,
    ...
    std::string cstListString(const std::string& name) const;
};
                ...
                const bool verifyNoRepeatedEntry)
{
    ...
    {
        const dealii::IndexSet& locallyOwnedDoFs = dofHandler.locally_owned_dofs();
        for (auto const & cell : dofHandler.active_cell_iterators())
        {
            // skip ghost cells or cells that are not at boundary
            // Note: at_boundary can accelerate the iterations but it cannot gaurantee the found vertices are at boundary.
            ...
            for (const auto vertex : cell->vertex_indices())
            {
                ...
                for (unsigned int j = 0; j < __cstSelectors.size(); ++j)
                {
                    ...
                    if (cstSelector.isSelectedPnt(point))
                    {
                        ...
                        for (const CstEntry<Tria>& cstEntry : cstSelector.cstInput())
                        {
                            ...
                            // extract global dof
                            const GlobalDoF dof = cell->vertex_dof_index(vertex, cstEntry.getDofAtPnt());
        ...
        for (; vertex_itr != tria.end_vertex(); ++vertex_itr)
        {
            for (unsigned int j = 0; j < __cstSelectors.size(); ++j)
            {
                ...
                if (selectorPtr->isSelectedPnt(vertex))
                {
                    ...
                    selectorPtr->createOutputByPoint(vertex_itr->vertex(), nDoFs);
                    hasFoundCsts     = true;
                }
            }
        }
    ...
    if constexpr (is_distributed)
    {
        hasFoundCsts = ::common::MPIInfo::syncBool<true>(hasFoundCsts,
