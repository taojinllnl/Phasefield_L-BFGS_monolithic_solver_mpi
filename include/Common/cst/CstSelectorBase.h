...
public:
    ...
    static bool isDoFValid(const GlobalDoF dof);
    bool hasNoCachedDoF(const CstEntry<Tria>& entryRef) const;
    
    
    virtual std::size_t expectedNumberOfCstPoints() const;
    virtual std::size_t expectedNumberOfCstEntries() const;
    
    std::size_t nCstPnts() const;
    std::size_t nCstEntries() const;
    
    void newCstInput(const std::vector<CstEntry<Tria>>& cstInput);
    
    
    const std::vector<CstEntry<Tria>>& cstInput() const;
    const std::vector<CstEntryResult<Tria>>& cstOutput() const;

    const std::vector<CstPointRecord>& cstPointRecordsFound() const;
    
    // Clear all cache data without changing cstInput.
    void clear();

    virtual bool isSelectedPnt(const ::dealii::Point<spacedim>& point) = 0;
    
    virtual std::unique_ptr<CstSelectorBase<Tria>> clone() const = 0;
    
    void init(const std::size_t nDoFs);
    
    std::vector<GlobalDoF>& addDoFs2Cache(const std::size_t nDoFs);
    void addDoF2Cache(const std::size_t nDoFs,
                      const GlobalDoF dof,
                      const CstEntry<Tria>& entryRef);
    
    void createOutputByPoint(const ::dealii::Point<spacedim>& point,
                             const std::size_t nDoFs);
    
    
    void updateValues(const std::size_t nDoFs);
};
}
